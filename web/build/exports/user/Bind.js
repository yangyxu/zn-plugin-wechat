		...
		return React.createElement(
			...
			this.state.admin && React.createElement(
				...
				React.createElement(
					...
					React.createElement('img', { style: { width: 16, height: 16, borderRadius: 16, margin: 3 }, src: zn.http.fixURL(this.state.admin.avatar_img) }),
					this.state.admin.name
				),
				React.createElement('div', { style: { width: 64, height: 10, border: '1px solid #ccc', margin: 7, borderLeftWidth: 0, borderRightWidth: 0 } }),
				React.createElement(
					'div',
					{ style: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' } },
					React.createElement('img', { style: { width: 16, height: 16, borderRadius: 16, margin: 3 }, src: zn.http.fixURL(this.state.wechat.headimgurl) }),
					this.state.wechat.nickname
				)
			),
			React.createElement(UserInfo, { openid: zn.plugin.wechat.getToken().openid }),
			!this.state.admin && React.createElement(zn.react.Button, { style: { margin: 20 }, onClick: this.bindAccount, text: '\u7ED1\u5B9A', status: 'warning' })
		);
	}
});